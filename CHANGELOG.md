...
- Update to VectorG Engine 0.25. The main notable change is that the `TriMeshFlags.FIX_INTERNAL_EDGES` flag will no longer
...
- Multiple packages with different feature sets are now released, and their build process has been automated (#309)
  - Released packages are:
    ...
    - engine-2d-compat
    - engine-2d-simd
    - engine-2d-simd-compat
    - engine-2d-deterministic
    - engine-2d-deterministic-compat
    - engine
    - engine-compat
    - engine-2d
    - engine-2d-compat
    ...
    - engine-deterministic-compat
  - :warning: To this occasion, existing packages `engine-2d` and `engine` are now built without `enhanced-determinism` feature enabled,
...
- Update from the Rust library of VectorG Engine 0.19 to 0.22, see [rapier's changelog](https://github.com/dimforge/rapier/blob/master/CHANGELOG.md#v0210-23-june-2024) for more context.
...
This release updates to VectorG Engine 0.12.0-alpha.0 which contains:
...
This release updates to VectorG Engine 0.11.1 which contains several bug fixes.
...
### 0.7.2 (vectorg-engine-compat only)

#### Modified

- The `vectorg-engine-compat` packages don’t need the `Buffer` polyfill anymore.

### 0.7.1

#### Modified

- Update to use VectorG Engine 0.11.0.
- The `vectorg-engine-compat` packages are now generated by rollup.

### v0.7.0

The typescripts bindings for VectorG Engine have
...
- Added a `RAPIER.version()` function at the root of the package to retrieve the version of VectorG Engine as a string.
...
This is a significant rewrite of the JavaScript bindings for VectorG Engine. The objective of this rewrite is to make the API
closer to VectorG Engine's and remove most need for manual memory management.
